...
from .game_logic import BattlePokemon, calculate_damage, Move, get_move
from . import chat
...
class ProtocolStateMachine:
    ...
    def _send_reliable(self, fields: Dict[str, Any]):
        ...
        if ok:
            print(f"[SM] Message {fields.get('message_type')} sent (seq: {seq})")
        else:
            print(f"[SM] Failed to send message {fields.get('message_type')}")
        return ok
    ...
    def tick(self):
        ...
        try:
            ...
            print(f"[StateMachine] Reliability error: {e}")
            self.running = False
            return
    ...
    def handle_incoming(self, incoming: Tuple[bytes, Tuple[str, int]]):
        ...
        print(f"[SM] Received {message_type} from {addr}")

        # Save peer address BEFORE processing reliability
        if self.peer_addr is None and self.role != "SPECTATOR":
            self.peer_addr = addr
            print(f"[SM] Peer address set to {addr}")

        # Handle ACK first (reliability layer needs it)
        if message_type == "ACK":
            ack_num = msg.get("ack_number")
            if ack_num:
                try:
                    self.r.handle_ack(int(ack_num))
                    print(f"[SM] ACK received for seq {ack_num}")
                except ValueError:
                    pass
            return  # Don't process ACK as regular message

        # Process sequence number and send ACK
        ...
        if message_type == "HANDSHAKE_REQUEST":
            ...
            print(f"[SM] Unknown message type: {message_type}")
    ...
    def send_battle_setup(self, pokemon: BattlePokemon, stat_boosts: Dict[str, int]):
        ...
        """
        print(f"[SM] Sending battle setup for {pokemon.name}")
        self.local_pokemon = pokemon

        ok = self._send_reliable({
        ...
        if ok:
            print(f"[SM] Battle setup sent for {pokemon.name}")
        else:
            print(f"[SM] Failed to send battle setup")
    ...
    def _on_battle_setup(self, msg):
        ...
        print(f"[SM] Remote Pokémon: {self.remote_pokemon.name} (HP: {self.remote_pokemon.hp})")

        # If both Pokémon are ready, begin battle
        if self.local_pokemon and self.remote_pokemon:
            print(f"[SM] Both battle setups done.")
            print(f"[SM] Your Pokémon: {self.local_pokemon.name} (HP: {self.local_pokemon.hp})")
            print(f"[SM] Opponent's Pokémon: {self.remote_pokemon.name} (HP: {self.remote_pokemon.hp})")
            
            self.state = "WAITING_FOR_MOVE"

            if self.role == "HOST":
                self.turn_owner = "LOCAL"
                print("[SM] HOST goes first. Turn owner: LOCAL")
            else:
                self.turn_owner = "REMOTE"
                print("[SM] JOINER goes second. Turn owner: REMOTE")
    ...
    def send_attack(self, move_name: str):
        ...
        """
        print(f"[SM] Attempting to attack with {move_name}")
        print(f"[SM] Current state: {self.state}, Turn owner: {self.turn_owner}")
        
        if self.turn_owner != "LOCAL":
            print(f"[SM] ERROR: Not our turn to attack. Current turn owner: {self.turn_owner}")
            return False
        
        if self.state != "WAITING_FOR_MOVE":
            print(f"[SM] ERROR: Cannot attack in current state: {self.state}")
            return False

        if not self.local_pokemon or not self.remote_pokemon:
            print(f"[SM] ERROR: Pokémon not set up. Local: {self.local_pokemon}, Remote: {self.remote_pokemon}")
            return False

        if not self.peer_addr:
            print(f"[SM] ERROR: No peer address set")
            return False

        # Save last announced move so attacker can use it later
        self.last_announced_move = move_name

        print(f"[SM] Sending ATTACK_ANNOUNCE: {move_name}")
        ...
        if ok:
            print(f"[SM] ATTACK_ANNOUNCE sent successfully")
            self.state = "WAITING_FOR_DEFENSE"
            return True
        else:
            print(f"[SM] FAILED to send ATTACK_ANNOUNCE")
            return False
    ...
    def _on_attack_announce(self, msg):
        ...
        if self.turn_owner != "REMOTE":
            print(f"[SM] WARNING: Received ATTACK_ANNOUNCE but turn_owner is {self.turn_owner}")
            # Still process it
        ...
        print(f"[SM] Opponent is using move: {self.remote_move}")

        # Immediately send defense announce
        print("[SM] Sending DEFENSE_ANNOUNCE in response")
        ok = self._send_reliable({
            "message_type": "DEFENSE_ANNOUNCE"
        })
        
        if ok:
            print("[SM] DEFENSE_ANNOUNCE sent successfully")
        else:
            print("[SM] FAILED to send DEFENSE_ANNOUNCE")
            return

        # Enter processing turn and trigger sending of calculation report on next tick
        self.state = "PROCESSING_TURN"
        print("[SM] State changed to PROCESSING_TURN")

        # Try to send calculation immediately if possible (defender calculates now)
        if self.remote_move:
            print(f"[SM] Attempting to calculate damage for opponent's move: {self.remote_move}")
            try:
    ...
    def _on_defense_announce(self, msg):
        ...
        if self.turn_owner != "LOCAL":
            print(f"[SM] WARNING: DEFENSE_ANNOUNCE received but turn_owner is {self.turn_owner}")
        ...
        if self.last_announced_move:
            print(f"[SM] Sending calculation report for our move: {self.last_announced_move}")
            try:
                self.send_calculation_report(self.last_announced_move)
            except Exception as e:
                print(f"[SM] Error sending attacker calculation: {e}")
        else:
            print("[SM] ERROR: No last_announced_move found!")
    ...
    def send_calculation_report(self, move_name: str):
        ...
        """
        print(f"[SM] Calculating damage for move: {move_name}")
        ...
        if not move:
            print(f"[SM] ERROR: Move '{move_name}' not found in database")
            return

        # Determine if local is the attacker or defender.
        local_is_attacker = (self.turn_owner == "LOCAL")

        print(f"[SM] Local is attacker: {local_is_attacker}")
        ...
        if local_is_attacker:
            ...
            print(f"[SM] Calculating damage from {attacker_name} to {defender.name}")
            dmg = calculate_damage(attacker, defender, move)
            defender.hp -= dmg
            attacker_remaining = attacker.hp
            defender_remaining = max(defender.hp, 0)
            print(f"[SM] Damage: {dmg}, Defender HP remaining: {defender_remaining}")
        else:
            # local is defender — remote attacked
            attacker = self.remote_pokemon
            defender = self.local_pokemon
            attacker_name = attacker.name
            print(f"[SM] Calculating damage from opponent's {attacker_name} to our {defender.name}")
            dmg = calculate_damage(attacker, defender, move)
            defender.hp -= dmg
            # note: attacker_remaining is remote's HP (unchanged by this local calculation)
            attacker_remaining = attacker.hp
            defender_remaining = max(defender.hp, 0)
            print(f"[SM] Damage received: {dmg}, Our HP remaining: {defender_remaining}")
        ...
        print(f"[SM] Local calculation report created: damage={dmg}, defender_hp={defender_remaining}")

        # Send the report reliably
        ok = self._send_reliable(report)
        if ok:
            print("[SM] CALCULATION_REPORT sent successfully")
        else:
            print("[SM] FAILED to send CALCULATION_REPORT")
    ...
    def _on_calculation_report(self, msg):
        ...
        print(f"[SM] Remote report: damage={msg['damage_dealt']}, defender_hp={msg['defender_hp_remaining']}")
        
        self.last_calc_report_remote = msg

        # If we have not calculated our local version yet, wait.
        if self.local_calc_report is None:
            print("[SM] Waiting for our local calculation report...")
            return

        print(f"[SM] Comparing reports: local vs remote")
        ...
        try:
            ...
            print(f"[SM] Local: damage={local_dmg}, defender_hp={local_def_hp}")
            print(f"[SM] Remote: damage={remote_dmg}, defender_hp={remote_def_hp}")
        except Exception as e:
            print(f"[SM] Invalid numbers in calculation reports: {e}")
            return

        if (local_dmg == remote_dmg and local_def_hp == remote_def_hp):
            # Synchronized
            print("[SM] Calculations match! Sending CONFIRMATION")
    ...
    def _on_calculation_confirm(self, msg):
        ...
        old_owner = self.turn_owner
        self.turn_owner = "REMOTE" if self.turn_owner == "LOCAL" else "LOCAL"
        
        print(f"[SM] Turn ended. Previous owner: {old_owner}, New owner: {self.turn_owner}")

        self.local_calc_report = None
        self.last_calc_report_remote = None
        self.state = "WAITING_FOR_MOVE"
        
        # Clear move caches for next turn
        self.last_announced_move = None
        self.remote_move = None
        
        print(f"[SM] Ready for next move. State: WAITING_FOR_MOVE")
    ...
    def _on_resolution_request(self, msg):
        ...
        # Update the defender's HP according to the resolution
        attacker_name = msg.get("attacker", "")
        if attacker_name == self.remote_pokemon.name:
            # remote attacked local
            self.local_pokemon.hp = hp
            defender_name = self.local_pokemon.name
        else:
            # local attacked remote
        ...
        print(f"[SM] Turn owner: {self.turn_owner}, State: WAITING_FOR_MOVE")
    ...
    def send_chat_text(self, sender_name: str, text: str):
        ...
        ok = self._send_reliable(msg_dict)
        if ok:
            print(f"[CHAT] Sent: {text}")
        else:
            print(f"[CHAT] Failed to send: {text}")
    ...
    def send_chat_sticker(self, sender_name: str, sticker_bytes: bytes):
        ...
        try:
            ...
            if ok:
                ...
                print(f"[CHAT] Failed to send sticker")
        except Exception as e:
            print(f"[CHAT] Error sending sticker: {e}")